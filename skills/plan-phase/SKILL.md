...
- Match each per-phase command to the behavior and files changed by that phase. Do not add a repository-wide, release-wide, or full regression suite to an ordinary implementation phase merely as a safety blanket. Such a suite belongs at the final release boundary, unless this phase itself is that boundary or the plan names a measured dependency gap that makes narrower verification unsafe.
- Before repeating an expensive command from an earlier phase or attempt, inspect the available harness verification report and sealed release evidence. Reuse valid evidence when the candidate bytes, command, environment fingerprint, and authority are unchanged; otherwise state the concrete invalidation reason in the plan. A timestamp, a new planning attempt, or reviewer preference alone is not an invalidation reason.
- Test-selection or affected-test products may accelerate the inner loop, but deselection is not correctness evidence when tests depend on files read as data, subprocesses, services, or external state that the selector does not track. Keep explicit tests for those boundaries and retain the canonical full suite at the release boundary.
